# Test technique — Intégrateur / Développeur web

Bienvenue, et merci de prendre le temps. Ce test sert à voir **comment tu construis une landing page réelle**, du brief au site déployé. Ce n'est pas un piège : on veut voir ta façon de travailler, pas te coller.

---

## 1. Le contexte

Tu intègres une petite équipe qui livre des **landing pages qui convertissent** à des clients (commerces, coachs, créateurs, services). Le quotidien : on reçoit un brief + du contenu, on construit un site **propre, beau, rapide, qui vend**, et on le met en ligne. Vite.

Ce test reproduit exactement ça. On te donne un client fictif, sa marque, et tout son contenu. **À toi de le transformer en landing.**
...
1. **Une URL en ligne** (déploie sur Vercel ou Netlify, gratuit, ~5 min). Le site doit charger et marcher.
2. **Un dépôt Git** (GitHub, public ou en nous invitant). On regarde **aussi ton historique de commits** : des commits clairs et progressifs valent mieux qu'un seul gros « final ».
3. **Un README court** (½ page max) qui répond à :
...
Une question sur le brief ? **Pose-la** — savoir poser la bonne question au bon moment fait partie du métier, et c'est mieux vu que de partir dans la mauvaise direction en silence.

Bon courage. On a hâte de voir.
